...
var c=this;
$pdk.plugin.freewheel.admanager.callbacks[a]=function(){if(arguments[0].type==="201"){arguments[0].type="defaultImpression"
}if(arguments&&arguments.length){if(arguments[0].adId){var f=c.getSlotByCustomId(arguments[0].slotCustomId);
var e=c.getAdInfo(f,""+arguments[0].adId,null,null);
arguments[0].adInstance=e;
arguments[0].adInstance.slot=f
}else{if(arguments&&arguments.length&&arguments[0].slotCustomId){arguments[0].slot=c.getSlotByCustomId(arguments[0].slotCustomId)
...
},setAdVolume:function(a){if(this.proxy&&this.proxy.volume){this.proxy.volume=a/100
}},setVolume:function(a){this.setAdVolume(a)
...
},_onAdEvent:function(a){if(this._isLinearSlot(a.slot)){console.error("onAdEvent: "+a.adInstance.getAdId()+"\t"+a.errorCode+"\t"+a.subType)
...
},_mediaMuteSet:function(c){if(!this.adManager){return
}if(c){this.adManager.setAdVolume(0)
}else{var b=this.controller.callFunction("getVolume",[]);
if(b){this.adManager.setAdVolume(b)
}else{var a=70;
this.adManager.setAdVolume(a)
...
$pdk.ns("$pdk.plugin.freewheel.slotmanager");
$pdk.plugin.freewheel.slotmanager.MultiClipSlotManager=$pdk.extend($pdk.plugin.freewheel.slotmanager.SlotManager,{loaded:false,started:false,constructor:function(a,d,b,c){$pdk.plugin.freewheel.slotmanager.SlotManager.call(this,a,d);
...
}},start:function(a){this.loaded=true;
this.started=false;
this.playlist=a;
this.index=-1;
this.slot=this.adManager.getSlotByCustomId(a.clips[0].baseClip.guid);
this.log("starting slot with "+a.clips.length+" ads",tpConsts.INFO);
this.slot.play()
},stop:function(){if(this.slot){this.slot.stop()
}},next:function(a){this.loaded=true;
this.started=false;
this.clip=a;
if(this.slotEnded&&this.index<this.playlist.clips.length){this._doError();
if(this.index===this.playlist.clips.length){this.clip=null;
this.playlist=null
...
}if(this._isLinearSlot(a.slot)){this.slotEnded=false
}},_onSlotEnded:function(b){if(this._isLinearSlot(b.slot)){this.slotEnded=true;
if(this.playlist&&this.playlist.clips){var a=this.playlist.clips.length-this.index-1;
if(a>0){this.log("Slot Ended with "+a+" unused clips.",tpConsts.INFO);
if(this.loaded){this.clip=this.playlist.clips[this.index];
this._doError()
}}}this._stopProgressTimer();
this.log("Slot Ended",tpConsts.INFO);
clearTimeout(this._timeoutId)
}},_doError:function(){this.index++;
this.controller.sendError({clip:this.clip,title:"Unused ad from slot"})
},_isLinearSlot:function(a){return(a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_PREROLL||a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_MIDROLL||a.getTimePositionClass()===tv.freewheel.SDK.TIME_POSITION_CLASS_POSTROLL)
},_onAdTimeout:function(a){this._endClip()
},_onAdEvent:function(a){if(this._isLinearSlot(a.slot)){if(a.subType==="adInitiated"){}else{if(a.subType===tv.freewheel.SDK.EVENT_ERROR){this._onAdError(a)
}else{if(a.subType===tv.freewheel.SDK.EVENT_AD_PAUSE){this._mediaPauseSet(true)
}}}}},_onAdImpression:function(b){clearTimeout(this._timeoutId);
var c=b.adInstance.slot||b.adInstance.getSlot();
var a=c.getTimePositionClass();
if(["PREROLL","MIDROLL","POSTROLL"].indexOf(a)>=0){this.index++;
this.adId=b.adInstance.adId||b.adInstance.getAdId();
this._startClip()
}},_onAdComplete:function(a){var b=a.adInstance.slot||a.adInstance.getSlot();
if(this.isTemporalSlot(b)){this._endClip()
}},_onAdError:function(a){this._onLinearSlotError(a)
},_onLinearSlotError:function(a){this.clipError=true;
if(!this.loaded&&!this.started){}else{if(this.started){this.log("Ending clip that has started and errored out...",tpConsts.INFO);
this._endClip()
}else{this.log("Ignoring clip that never started and errored out...",tpConsts.INFO)
}}},_startTimeout:function(){var a=this;
this._timeoutId=setTimeout(function(){a.skipCurrentAd()
},this.timeout)
},_startClip:function(){this.clip=this.playlist.clips[this.index];
if(!this.clip){this.slot.stop();
return
}this.clipError=false;
...
this.isLive=this._onReleaseStartGetIsLive(d)||this.isLive;
this.hasPrebuffered=false;
var c=d.data;
...
this.submitRequest()
},_onReleaseStartGetIsLive:function(b){for(var a in b.data.baseClips){if(!b.data.baseClips[a].isAd){return b.data.baseClips[a].expression.toLowerCase()==="nonstop"
}}return false
...
this.cleanup()
},onClearCurrentRelease:function(a){this.cleanup()
